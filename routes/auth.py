...
def login():
    if 'user_id' in session:
        ...
        if rol == 'Administrador':
            ...
            return redirect(url_for('admin.gerente_dashboard'))
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user[2].strip(), password_form):
            ...
            # Farmacéutico y Gerente deben coincidir con la sucursal configurada
            if rol in ('Farmaceutico', 'Gerente') and id_sucursal != SUCURSAL_ID:
            ...
            session['rol']              = rol
            session['id_sucursal_user'] = id_sucursal
            session['carrito']          = []
